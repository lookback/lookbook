...
const minify = require('./postcss-cssnano-default');
 ...
 */
module.exports = postcss.plugin('postcss-extract-media-query', (options) => {
  const opts = {
    prepend: null,
    entry: null,
    output: {
    ...
    Object.keys(newAtRules).forEach((key) => {
      ...
      if (opts.output.path) {
        ...
        if (opts.minimize === true) {
          const newRootMinimized = postcss(minify()).process(
            newRoot.toString(),
            { from: newFilePath }
          ).root;

          fs.writeFileSync(newFilePath, newRootMinimized.toString());
        } else {
          fs.writeFileSync(newFilePath, newRoot.toString());
        }

        if (opts.stats === true) {
          console.log('[extracted media query]', newFile);
        }
      }
